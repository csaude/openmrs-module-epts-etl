...
- *objectFields* the list of object fields. Note that each field is defined by:
  ...
  - (5) **dataType** an optional token to specify the data type for value. By default, the type will match the final expression type from the transformer. Supported types: int, long, double, string, date, boolean;
  - (6) **overrideTriggerValue** the value that triggers the override mechanism. If the transformed field value equals to this value, the  transformed value will be replaced by the configured "defaultValue"      
...
-  **mapping** is used to manually map the dataSource for specific fields in the dst table. The manual mapping is necessary if the dst field could not be automatically mapped because it does not appear in any dataSource in the srcConf. The relevant field for each mapping are:
   ...
   - (8) **transformer** a transformer enables transformation of "value". A transformation is needed when the "value" is an expression or needs a complex transformation. There are 4 types of transformers, namely:
...
- **childDst**: Defines additional destination configurations representing child records that must be created after the main destination record is inserted or updated. These configurations are used when the migration of a main entity requires the creation of related dependent records. For example, when migrating a person record, it may also be necessary to create related records such as person_attribute, person_name, or other tables that reference the person_id generated during the migration. Each childDst configuration supports the following properties:
  - *srcObjectDataSourceName*: The name of the source data source (defined in srcConf) that provides the source objects used to create the child destination records.
  - *srcObjectCondition*: An optional condition used to select the appropriate source object(s) from the data source. Only the objects that satisfy this condition will be used to populate the child destination table.
...
- **childDst**: related records wich must be migrated under this dst Object. Usualy these records are childred of the main migrated record. Each child can configure the following variables:
  - *srcObjectDataSourceName*: the specific datasource from the srcConf which will be used as source for this dst object;
  - *srcObjectCondition*: the condition to filter records from the datasource object list; 
