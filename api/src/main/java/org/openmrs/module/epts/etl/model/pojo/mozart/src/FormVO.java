...
public class FormVO extends AbstractDatabaseObject implements DatabaseObject {
	...
	public Object[] getInsertParamsWithoutObjectId() {
		Object[] params = { this.encounterId, this.encounterUuid, this.formId, this.encounterType, this.patientUuid,
	...
	public Object[] getInsertParamsWithObjectId() {
		Object[] params = { this.id, this.encounterId, this.encounterUuid, this.formId, this.encounterType, this.patientUuid,
		        this.createdDate, this.encounterDate, this.changeDate, this.locationUuid, this.sourceDatabase };
		return params;
	}
	...
	public Object[] getUpdateParams() {
		Object[] params = { this.id, this.encounterId, this.encounterUuid, this.formId, this.encounterType, this.patientUuid,
		        this.createdDate, this.encounterDate, this.changeDate, this.locationUuid, this.sourceDatabase, this.id };
		return params;
	}
	...
	public String getUpdateSQL() {
		return "UPDATE form SET id = ?, encounter_id = ?, encounter_uuid = ?, form_id = ?, encounter_type = ?, patient_uuid = ?, created_date = ?, encounter_date = ?, change_date = ?, location_uuid = ?, source_database = ? WHERE id = ? ";
	}
	...
	public String generateInsertValuesWithObjectId() {
		return "" + (this.id) + "," + (this.encounterId) + ","
		        + (this.encounterUuid != null ? "\"" + utilities.scapeQuotationMarks(encounterUuid) + "\"" : null) + ","
		        + (this.formId) + "," + (this.encounterType) + ","
		        + (this.patientUuid != null ? "\"" + utilities.scapeQuotationMarks(patientUuid) + "\"" : null) + ","
		        + (this.createdDate != null ? "\"" + DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(createdDate) + "\"" : null)
