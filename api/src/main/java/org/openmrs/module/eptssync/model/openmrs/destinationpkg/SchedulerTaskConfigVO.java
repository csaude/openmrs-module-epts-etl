...
public class SchedulerTaskConfigVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.schedulableClass = rs.getString("schedulable_class") != null ? rs.getString("schedulable_class").trim() : null;
		this.startTime =  rs.getTimestamp("start_time") != null ? new java.util.Date( rs.getTimestamp("start_time").getTime() ) : null;
		this.startTimePattern = rs.getString("start_time_pattern") != null ? rs.getString("start_time_pattern").trim() : null;
	...
	public String generateInsertValues(){ 
 		return ""+(this.name != null ? "\""+name+"\"" : null) + "," + (this.description != null ? "\""+description+"\"" : null) + "," + (this.schedulableClass != null ? "\""+schedulableClass+"\"" : null) + "," + (this.startTime != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(startTime)  +"\"" : null) + "," + (this.startTimePattern != null ? "\""+startTimePattern+"\"" : null) + "," + (this.repeatInterval) + "," + (this.startOnStartup) + "," + (this.started) + "," + (this.createdBy == 0 ? null : this.createdBy) + "," + (this.dateCreated != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateCreated)  +"\"" : null) + "," + (this.changedBy == 0 ? null : this.changedBy) + "," + (this.dateChanged != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateChanged)  +"\"" : null) + "," + (this.uuid != null ? "\""+uuid+"\"" : null) + "," + (this.lastExecutionTime != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(lastExecutionTime)  +"\"" : null); 
	} 
	...
	public void loadDestParentInfo(Connection conn) throws ParentNotYetMigratedException, DBException {
		...
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.UsersVO.class, this.createdBy, true, conn); 
		this.createdBy = 0;
