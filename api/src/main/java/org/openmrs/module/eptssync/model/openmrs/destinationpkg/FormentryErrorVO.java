...
public class FormentryErrorVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.error = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("error") != null ? rs.getString("error").trim() : null);
		this.errorDetails = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("error_details") != null ? rs.getString("error_details").trim() : null);
	...
	public String generateInsertValues(){ 
 		return ""+(this.formData != null ? "\""+ utilities.scapeQuotationMarks(formData)  +"\"" : null) + "," + (this.error != null ? "\""+ utilities.scapeQuotationMarks(error)  +"\"" : null) + "," + (this.errorDetails != null ? "\""+ utilities.scapeQuotationMarks(errorDetails)  +"\"" : null) + "," + (this.creator == 0 ? null : this.creator) + "," + (this.dateCreated != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateCreated)  +"\"" : null); 
	} 
