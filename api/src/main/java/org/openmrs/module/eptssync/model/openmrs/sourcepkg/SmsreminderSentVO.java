...
public class SmsreminderSentVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.cellNumber = rs.getString("cell_number") != null ? rs.getString("cell_number").trim() : null;
		this.alertDate =  rs.getTimestamp("alert_date") != null ? new java.util.Date( rs.getTimestamp("alert_date").getTime() ) : null;
		this.message = rs.getString("message") != null ? rs.getString("message").trim() : null;
	...
	public String generateInsertValues(){ 
 		return ""+(this.cellNumber != null ? "\""+cellNumber+"\"" : null) + "," + (this.alertDate != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(alertDate)  +"\"" : null) + "," + (this.message != null ? "\""+message+"\"" : null) + "," + (this.remainDays) + "," + (this.patientId == 0 ? null : this.patientId) + "," + (this.status != null ? "\""+status+"\"" : null) + "," + (this.created != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(created)  +"\"" : null); 
	} 
