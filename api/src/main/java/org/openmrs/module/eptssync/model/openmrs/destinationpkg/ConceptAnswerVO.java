...
public class ConceptAnswerVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public String generateInsertValues(){ 
 		return ""+(this.conceptId == 0 ? null : this.conceptId) + "," + (this.answerConcept == 0 ? null : this.answerConcept) + "," + (this.answerDrug == 0 ? null : this.answerDrug) + "," + (this.creator == 0 ? null : this.creator) + "," + (this.dateCreated != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateCreated)  +"\"" : null) + "," + (this.uuid != null ? "\""+ utilities.scapeQuotationMarks(uuid)  +"\"" : null) + "," + (this.sortWeight); 
	} 
	...
	public void loadDestParentInfo(Connection conn) throws ParentNotYetMigratedException, DBException {
		...
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.ConceptVO.class, this.answerConcept, true, conn); 
		this.answerConcept = 0;
		if (parentOnDestination  != null) this.answerConcept = parentOnDestination.getObjectId();
 
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.DrugVO.class, this.answerDrug, true, conn); 
		this.answerDrug = 0;
