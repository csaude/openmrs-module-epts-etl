...
public class LogicRuleTokenVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.className = rs.getString("class_name") != null ? rs.getString("class_name").trim() : null;
	...
	public String generateInsertValues(){ 
 		return ""+(this.creator == 0 ? null : this.creator) + "," + (this.dateCreated != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateCreated)  +"\"" : null) + "," + (this.changedBy == 0 ? null : this.changedBy) + "," + (this.dateChanged != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateChanged)  +"\"" : null) + "," + (this.token != null ? "\""+token+"\"" : null) + "," + (this.className != null ? "\""+className+"\"" : null) + "," + (this.state != null ? "\""+state+"\"" : null) + "," + (this.uuid != null ? "\""+uuid+"\"" : null); 
	} 
	...
	public void loadDestParentInfo(Connection conn) throws ParentNotYetMigratedException, DBException {
		...
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.sourcepkg.PersonVO.class, this.changedBy, true, conn); 
		this.changedBy = 0;
		if (parentOnDestination  != null) this.changedBy = parentOnDestination.getObjectId();
 
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.sourcepkg.PersonVO.class, this.creator, false, conn); 
		this.creator = 0;
