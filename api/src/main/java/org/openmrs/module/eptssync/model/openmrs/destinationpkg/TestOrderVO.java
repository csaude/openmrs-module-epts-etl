...
public class TestOrderVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.laterality = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("laterality") != null ? rs.getString("laterality").trim() : null);
		this.clinicalHistory = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("clinical_history") != null ? rs.getString("clinical_history").trim() : null);
		this.frequency = rs.getInt("frequency");
		this.numberOfRepeats = rs.getInt("number_of_repeats");
	} 
	...
	public String generateInsertValues(){ 
 		return ""+(this.specimenSource == 0 ? null : this.specimenSource) + "," + (this.laterality != null ? "\""+ utilities.scapeQuotationMarks(laterality)  +"\"" : null) + "," + (this.clinicalHistory != null ? "\""+ utilities.scapeQuotationMarks(clinicalHistory)  +"\"" : null) + "," + (this.frequency == 0 ? null : this.frequency) + "," + (this.numberOfRepeats); 
	} 
	...
	public void loadDestParentInfo(Connection conn) throws ParentNotYetMigratedException, DBException {
		...
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.ConceptVO.class, this.specimenSource, true, conn); 
