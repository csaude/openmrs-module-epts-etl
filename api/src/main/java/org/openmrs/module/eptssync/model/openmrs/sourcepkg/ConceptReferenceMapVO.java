...
public class ConceptReferenceMapVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public String generateInsertValues(){ 
 		return ""+(this.creator == 0 ? null : this.creator) + "," + (this.dateCreated != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateCreated)  +"\"" : null) + "," + (this.conceptId == 0 ? null : this.conceptId) + "," + (this.uuid != null ? "\""+uuid+"\"" : null) + "," + (this.conceptReferenceTermId == 0 ? null : this.conceptReferenceTermId) + "," + (this.conceptMapTypeId == 0 ? null : this.conceptMapTypeId) + "," + (this.changedBy == 0 ? null : this.changedBy) + "," + (this.dateChanged != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateChanged)  +"\"" : null); 
	} 
	...
	public void loadDestParentInfo(Connection conn) throws ParentNotYetMigratedException, DBException {
		...
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.sourcepkg.ConceptMapTypeVO.class, this.conceptMapTypeId, false, conn); 
		this.conceptMapTypeId = 0;
		if (parentOnDestination  != null) this.conceptMapTypeId = parentOnDestination.getObjectId();
 
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.sourcepkg.ConceptReferenceTermVO.class, this.conceptReferenceTermId, false, conn); 
