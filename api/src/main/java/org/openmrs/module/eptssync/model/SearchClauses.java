...
/**
 * Classe auxiliar para representar as clausulas de uma pesquisa.
 * Uma clausula de pesquisa e composta por uma String, representando uma sequencia de clausulas SQL
 * e uma lista de parametros associada a essas clausula bem como uma sequencia de clausulas para a clausula FROM
 * 
 * @author JPBOANE
...
public class SearchClauses<T extends VO> {
	 ...
	 * 
	 * @param anotherFromClause
	 * 	      Ex: "INNER JOIN PESSOA ON PESSOA.SELF_ID = TABELA_BASE.PESSOA_ID ".
	 * 		  Onde TABELA_BASE e uma tabela previamente adicionada�a clausula FROM
	 */
	public void addToClauseFrom(String anotherFromClause){
		if (!utilities.stringHasValue(anotherFromClause)) return;
	...
	public void addToClauses(String condition){
		if (!utilities.stringHasValue(condition)) return;
	...
	/**
	 * Modifica o valor do atributo {@link #havingClauses} para o valor fornecido pelo par�metro <code>havingClauses</code>
	 * 
	...
	/**
	 * Faz replace de uma coluna na lista de colunas de selecção.
	 * Este método é útil, por exemplo quando uma subclasse de uma determinada classe pretende usar o SearchClause da super classe
	...
	public void removeFromClauseFrom(String clauseFromLine){
		...
		else throw new ForbiddenOperationException("A clausula ocorre mais de uma vez");
	}
			...
			for (ClauseFromElements c : l){
				if (utilities.isStringIn(joinTable,c.getFirstTab(),c.getSecondTab())){
					if (c.getFirstTab().equals(joinTable)){
						this.addToClauseFrom("INNER JOIN " + joinTable + " " + c.getFirstTabAlias() +  " ON " + joinCondition); 
						c.setFirstTab("");
						c.setFirstTabAlias("");
					}
					else{
						if (true) throw new ForbiddenOperationException("Actualmente o metodo nao suporta a juncao de clausulas cuja tabela de juncao da 2a clausula nao esta na primeira posicao!");
						/*
				...
				if (utilities.stringHasValue(c.getFirstTab()) ||utilities.stringHasValue(c.getSecondTab())) this.addToClauseFrom(c.toString());
			}
		}
	...
	public void generateAndAddInClause(String field, List<?> inValues){
		if (!utilities.arrayHasElement(inValues)) return;
