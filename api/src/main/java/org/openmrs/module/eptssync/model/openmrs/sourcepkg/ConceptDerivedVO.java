...
public class ConceptDerivedVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.rule = rs.getString("rule") != null ? rs.getString("rule").trim() : null;
		this.compileDate =  rs.getTimestamp("compile_date") != null ? new java.util.Date( rs.getTimestamp("compile_date").getTime() ) : null;
		this.compileStatus = rs.getString("compile_status") != null ? rs.getString("compile_status").trim() : null;
			} 
	...
	public String generateInsertValues(){ 
 		return ""+(this.rule != null ? "\""+rule+"\"" : null) + "," + (this.compileDate != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(compileDate)  +"\"" : null) + "," + (this.compileStatus != null ? "\""+compileStatus+"\"" : null) + "," + (this.className != null ? "\""+className+"\"" : null); 
	} 
