...
public class DrugOrderVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.dosingType = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("dosing_type") != null ? rs.getString("dosing_type").trim() : null);
		this.quantity = rs.getDouble("quantity");
		this.asNeededCondition = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("as_needed_condition") != null ? rs.getString("as_needed_condition").trim() : null);
		this.numRefills = rs.getInt("num_refills");
		this.dosingInstructions = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("dosing_instructions") != null ? rs.getString("dosing_instructions").trim() : null);
		...
		this.brandName = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("brand_name") != null ? rs.getString("brand_name").trim() : null);
		this.dispenseAsWritten = rs.getByte("dispense_as_written");
	} 
	...
	public String generateInsertValues(){ 
 		return ""+(this.drugInventoryId == 0 ? null : this.drugInventoryId) + "," + (this.dose) + "," + (this.asNeeded) + "," + (this.dosingType != null ? "\""+ utilities.scapeQuotationMarks(dosingType)  +"\"" : null) + "," + (this.quantity) + "," + (this.asNeededCondition != null ? "\""+ utilities.scapeQuotationMarks(asNeededCondition)  +"\"" : null) + "," + (this.numRefills) + "," + (this.dosingInstructions != null ? "\""+ utilities.scapeQuotationMarks(dosingInstructions)  +"\"" : null) + "," + (this.duration) + "," + (this.durationUnits == 0 ? null : this.durationUnits) + "," + (this.quantityUnits == 0 ? null : this.quantityUnits) + "," + (this.route == 0 ? null : this.route) + "," + (this.doseUnits == 0 ? null : this.doseUnits) + "," + (this.frequency == 0 ? null : this.frequency) + "," + (this.brandName != null ? "\""+ utilities.scapeQuotationMarks(brandName)  +"\"" : null) + "," + (this.dispenseAsWritten); 
	} 
	...
	public void loadDestParentInfo(Connection conn) throws ParentNotYetMigratedException, DBException {
		...
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.ConceptVO.class, this.doseUnits, true, conn); 
		this.doseUnits = 0;
		if (parentOnDestination  != null) this.doseUnits = parentOnDestination.getObjectId();
 
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.ConceptVO.class, this.durationUnits, true, conn); 
		this.durationUnits = 0;
		if (parentOnDestination  != null) this.durationUnits = parentOnDestination.getObjectId();
 
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.OrderFrequencyVO.class, this.frequency, true, conn); 
		this.frequency = 0;
		if (parentOnDestination  != null) this.frequency = parentOnDestination.getObjectId();
 
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.ConceptVO.class, this.quantityUnits, true, conn); 
		...
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.OrdersVO.class, this.orderId, false, conn); 
		this.orderId = 0;
		if (parentOnDestination  != null) this.orderId = parentOnDestination.getObjectId();
 
		parentOnDestination = loadParent(org.openmrs.module.eptssync.model.openmrs.destinationpkg.DrugVO.class, this.drugInventoryId, true, conn); 
