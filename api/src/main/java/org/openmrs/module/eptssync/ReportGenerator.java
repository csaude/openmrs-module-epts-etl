...
public class ReportGenerator {
	...
	private static void produceReport(SyncTableConfiguration config, Connection conn) throws DBException {
		...
		String report = config.getTableName() + "," + (recordsOnMainTable + recordsOnStageTable) + "," + recordsOnMainTable + "," + recordsOnStageTable;
	...
	private static int countMissingParents(SyncTableConfiguration config, String tableName, Connection conn) throws DBException {
		String sql = "SELECT COUNT(*) as value FROM " + config.generateFullStageTableName() + " WHERE last_migration_try_err LIKE '%" + tableName + ":%'";
