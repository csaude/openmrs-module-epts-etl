...
public class IdgenLogEntryVO extends AbstractOpenMRSObject implements OpenMRSObject { 
	...
	public void load(ResultSet rs) throws SQLException{ 
		...
		this.comment = AttDefinedElements.removeStrangeCharactersOnString(rs.getString("comment") != null ? rs.getString("comment").trim() : null);
	} 
	...
	public String generateInsertValues(){ 
 		return ""+(this.source == 0 ? null : this.source) + "," + (this.identifier != null ? "\""+ utilities.scapeQuotationMarks(identifier)  +"\"" : null) + "," + (this.dateGenerated != null ? "\""+ DateAndTimeUtilities.formatToYYYYMMDD_HHMISS(dateGenerated)  +"\"" : null) + "," + (this.generatedBy == 0 ? null : this.generatedBy) + "," + (this.comment != null ? "\""+ utilities.scapeQuotationMarks(comment)  +"\"" : null); 
	} 
